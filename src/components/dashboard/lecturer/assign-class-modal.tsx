...
export function AssignClassModal({ studentIds, onClose, onSuccess, lecturerId }: AssignClassModalProps) {
  ...
  const fetchClasses = async () => {
    try {
      const availableClasses = await databaseService.getClassesByLecturer(lecturerId);
      setClasses(availableClasses.filter((c: Class) => c.availableSlots >= studentIds.length));
    } catch (error) {
              ...
              filteredClasses.map((cls) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Available: {cls.availableSlots}/{cls.capacity} slots
                    </p>
                  </div>
                  <Button
                    variant="outline"
